...
import { getSymbolFromCurrency } from '@/utils/format'
...
export const DebtSummary = ({ onAddNew }: Props) => {
  ...
  const renderCharts = () => {
    ...
    const legendItems = barData.map(item => ({
      ...
      label: item.label,
      value: `${getSymbolFromCurrency(currency)} ${item.value.toLocaleString()}`
    }))
            ...
            <View style={styles.insightsContainer}>
              ...
              <View style={[styles.insightCard]}>
                <View style={styles.insightContent}>
                  ...
                  <AmountCurrencyView
                    amount={summary.monthlyRepaymentTotal || 0}
                    currency={currency}
                    style={styles.insightTitle}
                  />
                  <Text style={styles.insightValue}>
                    Due{' '}
                    {summary.upcomingRepayment
                      ? new Date(
                          summary.upcomingRepayment.repayment.repayment_date
                        ).toLocaleDateString()
