...
export const addExpense = async (expense: ExpenseInput): Promise<number> => {
  try {
    ...
    const result = await db.runAsync(
      'INSERT INTO expenses (amount, currency, category_id, comment, date) VALUES (?, ?, ?, ?, ?)',
      [expense.amount, expense.currency, expense.category_id, expense.comment || null, expense.date]
    );
